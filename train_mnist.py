...
from arch import MinimalCNN_GAP_DO
...
def train_single_model(dataset, batch_size, model_id, metric, lambda_reg, epochs, model_save_path):
    ...
    # NHUT, 10 works is ok if no one else on system, otherwise reduce.
    ...
    model = MinimalCNN_GAP_DO().to(device)   # Assume model is defined elsewhere
    ...
    for epoch in range(epochs):
        ...
        for data, target in train_loader:
            # print(f"Processing batch for Model {model_id} in Epoch {epoch + 1}")  # Debug print
            ...
            weights = model.fc1.weight  # Get weights of the fully connected layer
...
# Nhut, lambda value for current parallel model set here
# epchos and num models passed as args below. 
...
if __name__ == "__main__":
    ...
    for lambda_reg in lambda_values:
        ...
        model_id = 0
        train_single_model(train_dataset, batch_size=64, model_id=model_id, metric='correlation', lambda_reg=lambda_reg, epochs=10,
                           model_save_path = f"test_mnist_weight_correlation_lambda{lambda_reg}_model{model_id}.pth")
